...
function ScoutFilter({ id, label, value, options, onChange }: { id: string; label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label className="filter-field"><span>{label}</span><select id={id} data-testid={id} value={value} onChange={event => onChange(event.target.value)}>{options.map(option => <option key={option} value={option}>{option === SCOUT_ALL ? 'All' : option}</option>)}</select></label>;
}

function SummaryTable({ label, rows, testId }: { label: string; rows: Array<{ name: string; summary: ScoutSummary }>; testId: string }) {
  return <div className="table-wrap"><table data-testid={testId}><thead><tr><th>{label}</th><th>SNAPS</th><th>% TOTAL</th><th>RUN %</th><th>PASS %</th><th>AVG YDS</th><th>SUCCESS %</th><th>EXPLOSIVE %</th><th>TOP SCHEME</th><th>TOP RUN</th><th>TOP PASS</th><th>BACKFIELD</th></tr></thead><tbody>{rows.map(row => <tr key={row.name}><td>{row.name}</td><td>{row.summary.count}</td><td>{pctText(row.summary.pctTotal)}</td><td>{pctText(row.summary.runPct)}</td><td>{pctText(row.summary.passPct)}</td><td>{decText(row.summary.avgGain)}</td><td>{pctText(row.summary.successRate)}</td><td>{pctText(row.summary.explosiveRate)}</td><td>{row.summary.topScheme}</td><td>{row.summary.topRun}</td><td>{row.summary.topPass}</td><td>{row.summary.topBackfield}</td></tr>)}</tbody></table></div>;
}

function ScoutVerdict({ verdict, tone }: { verdict: string; tone: 'run' | 'pass' | 'balanced' | 'none' }) {
  return <div className={`scout-verdict ${tone}`}><strong>{verdict}</strong></div>;
}
