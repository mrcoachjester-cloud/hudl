...
function LiveSpreadsheetPage({ data, setData }: { data: Dataset; setData: (data: Dataset) => void }) {
  ...
  const activeGame = data.schedule.find(game => game.id === activeGameId);
  ...
  useEffect(() => {
    ...
    async function loadLivePlays() {
      ...
      try {
        const rows = await getLivePlays(activeGameId);
        if (cancelled) return;

        const mapped: Play[] = rows.map(play => ({
          playNo: String(play.play_number).padStart(2, '0'),
          odk: play.odk ?? '',
          dn: play.down == null ? '' : String(play.down),
          dist: play.dist == null ? '' : String(play.dist),
          hash: play.hash ?? '',
          gnls: play.gnls == null ? '' : String(play.gnls),
        ...
        setForm(current => ({
          ...current,
          playNo: String(mapped.length + 1).padStart(2, '0'),
          yardLn: mapped.at(-1)?.yardLn ?? startingYardLine,
          gnls: '0',
          result: '',
        }));
      } catch (error) {
        console.error('Could not load live plays:', error);
        toast.notify('Could not load live data from Supabase');
      } finally {
  ...
  const update = (key: keyof Play, value: string) => {
  ...
  const addPlay = async () => {
    if (!activeGameId) {
      toast.notify('Select a game before adding live data');
      return;
    }

    if (!form.yardLn.trim()) {
      toast.notify('Enter the yard line after the snap');
      return;
    }

    if (calculatedGnls === null) {
      toast.notify('Use a signed yard line like -22 or 22');
      return;
    }

    if (!form.result.trim()) {
      toast.notify('Add a result before saving the snap');
      return;
    }

    const playNumber = live.length + 1;
    const gnls = calculatedGnls;

    try {
      const saved = await createLivePlay({
        gameId: activeGameId,
        playNumber,
        odk: normalizeOdk(form.odk),
        down: form.dn ? num(form.dn) : null,
        dist: form.dist ? num(form.dist) : null,
        hash: form.hash || null,
        gnls,
        yardLine: Number(normalizedFormYardLine),
        playType: form.type || null,
        result: form.result || null,
        offFormation: form.form || null,
        personnel: form.personnel || null,
        scheme: form.scheme || null,
        motion: form.motion || null,
        offPlay: form.offPlay || null,
        ballCarrier: form.carrier || null,
        defense: form.defense || null,
        playDir: form.dir || null,
        backfield: form.backfield || null,
      });

      const mapped: Play = {
        playNo: String(saved.play_number).padStart(2, '0'),
        odk: saved.odk ?? '',
        dn: saved.down == null ? '' : String(saved.down),
        dist: saved.dist == null ? '' : String(saved.dist),
        hash: saved.hash ?? '',
        gnls: saved.gnls == null ? '' : String(saved.gnls),
        carrier: saved.ball_carrier ?? '',
        yardLn: saved.yard_line == null ? '' : String(saved.yard_line),
        type: saved.play_type ?? '',
        result: saved.result ?? '',
        form: saved.off_formation ?? '',
        personnel: saved.personnel ?? '',
        scheme: saved.scheme ?? '',
        defense: saved.defense ?? '',
        motion: saved.motion ?? '',
        offPlay: saved.off_play ?? '',
        dir: saved.play_dir ?? '',
        backfield: saved.backfield ?? '',
      };

      const nextLive = [...live, mapped];
      ...
      setForm(current => ({
        ...current,
        playNo: String(nextLive.length + 1).padStart(2, '0'),
        yardLn: normalizedFormYardLine,
        gnls: '0',
        result: '',
      }));

      toast.notify(`Live snap saved · GN/LS ${formatGnls(calculatedGnls)}`);
    } catch (error) {
      console.error('Could not save live play:', error);
      toast.notify('Could not save live snap to Supabase');
    }
  };

  const updateLiveRow = async (
    displayIndex: number,
    key: keyof Play,
    value: string
  ) => {
    const actualIndex = live.length - 1 - displayIndex;
    const play = live[actualIndex];

    if (!play) return;

    const normalizedValue =
      key === 'odk' ? normalizeOdk(value) :
      key === 'yardLn' ? normalizeYardLine(value) :
      value;

    const nextLive = live.map((row, index) =>
      index === actualIndex
    ...
    if (key === 'yardLn') {
      ...
      const updated = recalculated[actualIndex];

      if (!updated) return;

      try {
        await updateLivePlay(play.playNo ? play.playNo : '', {
          yardLine: Number(normalizedValue),
          gnls: num(updated.gnls),
        });
        setData({ ...data, live: recalculated });
      } catch (error) {
        console.error('Could not update live play:', error);
        toast.notify('Could not update live snap');
      }

      return;
    }

    const fieldMap: Record<keyof Play, string> = {
      playNo: 'playNumber',
      odk: 'odk',
      dn: 'down',
      dist: 'dist',
    ...
    try {
      await updateLivePlay(play.playNo, {
        [fieldName]: value,
      } as any);
  ...
  const removeLiveRow = async (displayIndex: number) => {
    const actualIndex = live.length - 1 - displayIndex;
    const play = live[actualIndex];
    ...
    try {
      await deleteLivePlay(play.playNo);

      const nextLive = live.filter((_, index) => index !== actualIndex);
      ...
      console.error('Could not delete live play:', error);
      toast.notify('Could not delete live snap');
    }
  };
  ...
  const exportLiveCsv = () => {
    ...
    const opponent = activeGame?.opponent?.replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'game';
    download(
      `hudl-live-${opponent}.csv`,
      [headers.join(','), ...rows].join('\n')
    );

    toast.notify(`${live.length} live snaps exported`);
  };
  ...
  const yardLineField = (
    <div className="field">
      <label htmlFor="live-yardLn">Yard line</label>
      <input
        id="live-yardLn"
        className="input"
        value={form.yardLn}
        onChange={event => update('yardLn', event.target.value)}
        onBlur={() => update('yardLn', normalizeYardLine(form.yardLn))}
        placeholder="-22 or 22"
        inputMode="numeric"
        data-testid="input-live-yardLn"
      />
      <span className="field-hint">
        Negative = own · positive = opponent
      </span>
    </div>
  );
  ...
  const editCell = (
    ...
    options ? (
      <select
        ...
        value={value}
        onChange={event =>
          updateLiveRow(displayIndex, key, event.target.value)
        }
        aria-label={`Edit ${key} for snap ${displayIndex + 1}`}
        data-testid={`select-edit-live-${displayIndex}-${key}`}
      >
        {options.map(option => (
          <option key={option}>{option}</option>
        ))}
      ...
      <input
        ...
        onBlur={event =>
          ...
          updateLiveRow(
            displayIndex,
            key,
            normalizeYardLine(event.currentTarget.value)
          )
        }
        aria-label={`Edit ${key} for snap ${displayIndex + 1}`}
        data-testid={`input-edit-live-${displayIndex}-${key}`}
      />
    );
    ...
    return (
      <div className="content">
        <PageHead
          ...
          description="Choose a game from the global game selector before entering live data."
        />
        <Panel>
          <div className="empty">
            <Target size={30} />
            <h3>No game selected</h3>
            <p>Use the game selector at the top of the page.</p>
          </div>
        </Panel>
      </div>
    );
  ...
  return (
    <div className="content">
      <PageHead
        eyebrow={`Live game · ${activeGame?.opponent ?? 'Selected game'}`}
        title="Keep every snap in reach."
        description="Chart the game as it happens. Every Add Snap saves directly to Supabase."
        actions={
          <button
      ...
      <Panel>
        <SectionTitle
          title="Add a live snap"
          detail="Choose O, D, or K. GN/LS calculates from the previous spot."
        />

        {!live.length && (
          <div className="starting-yard-line">
            <div className="field">
              <label htmlFor="live-starting-yard-line">
                Starting yard line
              </label>
              <input
                id="live-starting-yard-line"
                className="input"
                value={startingYardLine}
                onChange={event =>
                  setStartingYardLine(event.target.value)
                }
                onBlur={() =>
                  setStartingYardLine(
                    normalizeYardLine(startingYardLine)
                  )
                }
                placeholder="-20"
                inputMode="numeric"
                data-testid="input-live-starting-yard-line"
              />
            </div>
            <span>
              Negative = your side · positive = opponent’s side.
            </span>
          </div>
        )}
        ...
        <div className="form-grid">
          ...
          <div className="field">
            <label htmlFor="live-gnls">GN/LS · calculated</label>
            <output
              id="live-gnls"
              className={`computed-value ${
                calculatedGnls !== null && calculatedGnls >= 0
                  ? 'positive'
                  : calculatedGnls !== null
                    ? 'negative'
                    : ''
              }`}
              data-testid="output-live-gnls"
            >
              {formatGnls(calculatedGnls)}
            </output>
            <span className="field-hint">
              From {previousYardLine}
            </span>
          </div>
        ...
        <div className="actions" style={{ marginTop: 17 }}>
          <button
            className="btn btn-green"
            onClick={addPlay}
            disabled={loading}
            data-testid="button-add-live-play"
          >
            <Plus /> Add snap <span style={{ opacity: 0.7 }}>↵</span>
          </button>
          ...
          >
            {loading ? 'Loading…' : `${live.length} live snaps tracked`}
          </span>
        </div>
      </Panel>

      <Panel style={{ marginTop: 14 }} pad={false}>
        <div style={{ padding: '21px 21px 0' }}>
          <SectionTitle
            title="Live game spreadsheet"
            detail={
              live.length
                ? `${live.length} snaps · click any cell to edit`
        ...
        {live.length ? (
          <div className="table-wrap">
            <table className="data-table live-sheet">
              <thead>
                <tr>
                  <th>Play</th>
                  <th>ODK</th>
                ...
                {live
                  ...
                  .map((play, i) => (
                    ...
                    >
                      ...
                      <td>
                        {editCell(i, 'offPlay', play.offPlay)}
                      </td>

                      <td>
                        {editCell(i, 'form', play.form)}
                      </td>

                      <td>
                        {editCell(i, 'carrier', play.carrier)}
                      </td>

                      <td>
                        {editCell(i, 'yardLn', play.yardLn)}
                      </td>
                      ...
                      >
                        {formatGnls(num(play.gnls))}
                      </td>

                      <td>
                        {editCell(i, 'result', play.result)}
                      </td>
                      ...
                      <td>
                        <button
                          className="btn btn-danger"
                          style={{ padding: 6 }}
                          onClick={() => removeLiveRow(i)}
                          aria-label={`Remove play ${play.playNo}`}
                          data-testid={`button-remove-live-${i}`}
                        >
                          <Trash2 size={13} />
                        </button>
                      </td>
          ...
          <div className="empty">
            ...
            <h3>No live snaps yet</h3>
            <p>
              Add a snap above. Saved snaps are stored in Supabase
              for this game.
            </p>
          </div>
        )}
