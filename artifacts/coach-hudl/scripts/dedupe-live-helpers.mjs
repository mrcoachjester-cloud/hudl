...
// Final safety net: if any exact report component still contains the free
// variable, replace only that prop rather than touching unrelated component data props.
...
// The deployed bundle still had a free `data` reference after the report-route
// cleanup. Make the fallback deterministic at the final transformation stage.
// Remove only previous module-level fallbacks generated by our scripts, then add
// exactly one. Local component variables named `data` are left untouched.
const dataFallbackPattern = /(^|\n)\s*const\s+data\s*=\s*safeLoad\(\);\s*(?=\n|$)/g;
const beforeDataFallbackCleanup = source;
source = source.replace(dataFallbackPattern, '$1');
if (source !== beforeDataFallbackCleanup) totalRemoved++;

const appExportIndex = source.lastIndexOf('export default App;');
if (appExportIndex >= 0 && !/\nconst\s+data\s*=\s*safeLoad\(\);\s*\nexport default App;\s*$/.test(source)) {
  source = source.slice(0, appExportIndex) + 'const data = safeLoad();\n' + source.slice(appExportIndex);
}
