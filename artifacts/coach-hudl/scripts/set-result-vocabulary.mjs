...
const realtimeEffect = "\n  // LIVE_REALTIME_INSTALLED: shared active game collaboration\n  useEffect(() => {\n    if (!isSupabaseConfigured || !data.activeGameId) return;\n    let cancelled = false;\n    const refresh = async () => { try { const rows = await getLivePlays(data.activeGameId); if (!cancelled) setData({ ...data, live: rows.map(livePlayToStandard) }); } catch {} };\n    void refresh();\n    const channel = supabase?.channel(`live-game:${data.activeGameId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'plays', filter: `game_id=eq.${data.activeGameId}` }, () => { void refresh(); }).subscribe();\n    return () => { cancelled = true; if (channel && supabase) void supabase.removeChannel(channel); };\n  }, [data.activeGameId]);\n";

function injectAfterSignature(text, functionName, effect) {
  const start = text.indexOf(`function ${functionName}(`);
  if (start < 0) return text;
  const signatureEnd = text.indexOf('{', text.indexOf(')', start));
  if (signatureEnd < 0) return text;
  if (text.slice(start, signatureEnd + 1).includes('LIVE_REALTIME_INSTALLED')) return text;
  return text.slice(0, signatureEnd + 1) + effect + text.slice(signatureEnd + 1);
}

if (!source.includes('LIVE_REALTIME_INSTALLED')) {
  source = injectAfterSignature(source, 'LiveSpreadsheetPage', realtimeEffect);
}
if (!source.includes('LIVE_REALTIME_INSTALLED')) {
  source = injectAfterSignature(source, 'LivePage', realtimeEffect);
}
