...
// Add the Supabase live-play helpers once. They use the existing `plays` table
// and the selected scheduled game's real database id.
...
const oldFootballImport = `import { getGames, getLivePlays, getScoutingSessions, getScoutingPlays, getSeasons, livePlayToStandard, scoutingPlayToStandard, type StandardPlay } from './lib/footballData';`;
const newFootballImport = `import { createGame, getGames, getLivePlays, getScoutingSessions, getScoutingPlays, getSeasons, livePlayToStandard, scoutingPlayToStandard, type StandardPlay } from './lib/footballData';`;
if (text.includes(oldFootballImport)) text = text.replace(oldFootballImport, newFootballImport);

const marker = `function AppShell({ children, data, setData }: { children: ReactNode; data: Dataset; setData: (data: Dataset) => void }) {`;
const helpers = `async function insertLivePlay(gameId: string, play: Play) {\n  if (!supabase || !gameId) return;\n  const { error } = await supabase.from('plays').insert({\n    game_id: gameId,\n    play_number: Number(play.playNo),\n    odk: normalizeOdk(play.odk),\n    down: Number(play.dn) || null,\n    dist: Number(play.dist) || null,\n    hash: play.hash || null,\n    gnls: Number(play.gnls) || 0,\n    yard_line: Number(play.yardLn) || null,\n    play_type: play.type || null,\n    result: play.result || null,\n    off_formation: play.form || null,\n    personnel: play.personnel || null,\n    scheme: play.scheme || null,\n    motion: play.motion || null,\n    off_play: play.offPlay || null,\n    ball_carrier: play.carrier || null,\n    defense: play.defense || null,\n    play_dir: play.dir || null,\n    backfield: play.backfield || null,\n  });\n  if (error) throw error;\n}\n\nasync function updateLivePlay(gameId: string, playNumber: number, play: Play) {\n  if (!supabase || !gameId) return;\n  const { error } = await supabase.from('plays').update({\n    odk: normalizeOdk(play.odk), down: Number(play.dn) || null, dist: Number(play.dist) || null, hash: play.hash || null,\n    gnls: Number(play.gnls) || 0, yard_line: Number(play.yardLn) || null, play_type: play.type || null, result: play.result || null,\n    off_formation: play.form || null, personnel: play.personnel || null, scheme: play.scheme || null, motion: play.motion || null,\n    off_play: play.offPlay || null, ball_carrier: play.carrier || null, defense: play.defense || null, play_dir: play.dir || null, backfield: play.backfield || null,\n  }).eq('game_id', gameId).eq('play_number', playNumber);\n  if (error) throw error;\n}\n\nasync function deleteLivePlay(gameId: string, playNumber: number) {\n  if (!supabase || !gameId) return;\n  const { error } = await supabase.from('plays').delete().eq('game_id', gameId).eq('play_number', playNumber);\n  if (error) throw error;\n}\n\n`;
if (!text.includes('async function insertLivePlay(') && text.includes(marker)) text = text.replace(marker, helpers + marker);

// Persist newly charted snaps in LivePage.
...
// The compact LivePage also supports add/remove; patch its add/remove paths.
const oldCompactAdd = `    setData(next);\n    setForm(current => ({ ...current, playNo: String(next.live.length + 1).padStart(2, '0'), yardLn: normalizedFormYardLine, gnls: '0', result: '' }));\n    toast.notify(\`Live snap added · GN/LS \${formatGnls(calculatedGnls)}\`);`;
if (text.includes(oldCompactAdd) && !text.includes('void insertLivePlay(data.activeGameId, next.live[next.live.length - 1])')) text = text.replace(oldCompactAdd, newLiveAdd);

// Ensure changing games always loads only that game's live data.
